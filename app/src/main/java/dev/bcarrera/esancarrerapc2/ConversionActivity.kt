...
class ConversionActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val amountEditText = EditText(this).apply {
            ...
            setPadding(32, 16, 16, 16)  // Espaciado para un mejor aspecto
        }
        ...
        val convertButton = Button(this).apply {
            text = "Convert"
            setPadding(24, 16, 24, 16)  // Padding en el botón para hacerlo más visible
            setOnClickListener {
                val amount = amountEditText.text.toString().toDoubleOrNull()
                if (amount != null) {
                    val fromCurrency = fromSpinner.selectedItem.toString()
                    val toCurrency = toSpinner.selectedItem.toString()
                    val result = TextView(this@ConversionActivity)
                    convertCurrency(fromCurrency, toCurrency, amount, result)
                } else {
        ...
        val resultTextView = TextView(this).apply {
            ...
            textSize = 18f
            gravity = Gravity.CENTER
            setPadding(16, 16, 16, 16)  // Padding: 16px a todos los lados
        }
    ...
    private fun convertCurrency(from: String, to: String, amount: Double, resultTextView: TextView) {
        val rates = mapOf(
